      ...
      {!selected ? (
        ...
        <>
          ...
          <section className="sequence-panel glass-panel">
            ...
            <div className="sequence-ladder">{[9,8,7,6,5,4,3,2,1,0].map(seq=><div className={"seq-node "+(seq===sequence?"current":"locked")} key={seq}><span>SEQ {seq}</span><div><b>{(pathway?.seq?.[9-seq]||(["Seer","Clown","Magician","Faceless","Marionettist","Bizarro Sorcerer","Scholar of Yore","Miracle Invoker","Attendant of Mysteries","The Fool"][9-seq]))}</b><small>{seq===sequence?"CURRENT":"LOCKED"} · {thresholds[9-seq]||0}h required</small></div><strong>{seq===sequence?"ACTIVE":"LOCKED"}</strong></div>)}</div>
