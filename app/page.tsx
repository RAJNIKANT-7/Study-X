...
function Timer(){
 ...
 return <main className="timer-page">
  ...
  <div className="timer-ring-wrap"><div className="timer-ring" style={{background:"conic-gradient(from -90deg, #f4f4f5 0%, #777 "+Math.max(.8,pct)+"%, rgba(255,255,255,.075) "+Math.max(.8,pct)+"%)"}}><div className="timer-disc"><div className="timer-mode-label">{run?"Session active":mode}</div><div className="timer-value">{fmt(left)}</div><div className="time-units"><span>HRS</span><span>MIN</span><span>SEC</span></div><div className="label-wrap"><button className="label-button" onClick={()=>setOpen(v=>!v)}>{label} <span>⌄</span></button>{open&&<div className="label-menu">{["Mathematics","Physics","Chemistry","Computer Science","Biology"].map(x=><button key={x} onClick={()=>{setLabel(x);setOpen(false)}}>{x}</button>)}</div>}</div></div></div></div>
  ...
  <div className="bottom-tools"><button>Tasks</button><button onClick={()=>setSound(v=>!v)}>{sound?"Sound on":"Sound off"}</button><button>Ambient</button><button>Study room</button></div>
