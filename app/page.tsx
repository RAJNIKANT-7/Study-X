...
function Timer(){
 ...
 useEffect(()=>{const id=setInterval(async()=>{try{const a=await fetch("/api/auth");if(!(await a.json()).loggedIn)return;const progress=JSON.parse(localStorage.getItem("study-x-progress")||"{\"seconds\":0,\"sessions\":0,\"daily\":{}}");await fetch("/api/sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({progress,tasks})})}catch{}},2000);return()=>clearInterval(id)},[tasks]);
...
function PathwayProgress(){
 const pathways=[["Fool","Lord of Mysteries","The Fool"],["Error","Lord of Mysteries","The Error"],["Door","Lord of Mysteries","The Door"],["Visionary","Lord of Mysteries","The Visionary"],["White Tower","Lord of Mysteries","The White Tower"],["Darkness","Lord of Mysteries","The Darkness"],["Death","Lord of Mysteries","The Death"],["Hanged Man","Lord of Mysteries","The Hanged Man"],["Sun","Lord of Mysteries","The Sun"]] as const;
 ...
 useEffect(()=>{const load=async()=>{try{const r=await fetch("/api/sync");if(r.ok){const x=await r.json();if(x.data?.progress)localStorage.setItem("study-x-progress",JSON.stringify(x.data.progress));setData(x.data?.progress||JSON.parse(localStorage.getItem("study-x-progress")||"{\"seconds\":0,\"sessions\":0,\"daily\":{}}"));setSelected(x.data?.progress?.pathway||null)}}catch{try{setData(JSON.parse(localStorage.getItem("study-x-progress")||"{\"seconds\":0,\"sessions\":0,\"daily\":{}}"))}catch{}}};load();const id=setInterval(load,3000);return()=>clearInterval(id)},[]);
 async function choose(name:string){if(selected)return;setSelected(name);try{const p={...data,pathway:name};setData(p);localStorage.setItem("study-x-progress",JSON.stringify(p));await fetch("/api/sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({progress:p,tasks:[]})})}catch{}}
 const hours=Math.floor((data.seconds||0)/3600),days=Object.keys(data.daily||{}).length;
 return <main className="pathways-page"><div className="page-heading"><span className="section-kicker">PROGRESS / PATHWAY</span><h1>Your Pathway</h1><p>Choose one pathway. Your choice is tied to this account and continues across devices.</p></div><section className="pathway-hero glass-panel"><div><span className="pathway-kicker">CURRENT PATH</span><h2>{selected?selected+" Pathway":"Choose your pathway"}</h2><p>{selected?"Continue your journey from any device using the same account. You cannot switch to another pathway on this account.":"Select one of the nine pathways below. Once selected, it is permanently linked to this account."}</p></div><div className="sequence-core"><span>SEQ.</span><b>9</b><small>START</small></div></section><div className="pathway-grid">{pathways.map((p,i)=>{const active=selected===p[0];return <article className={"pathway-card glass-panel "+(active?"unlocked":"")} key={p[0]}><div className="pathway-top"><span className="pathway-number">0{i+1}</span><span className={active?"unlocked-badge":"locked-badge"}>{active?"CURRENT":selected?"UNAVAILABLE":"AVAILABLE"}</span></div><h3>{p[0]}</h3><strong>{p[1]}</strong><p>{p[2]} pathway</p><button disabled={!!selected} onClick={()=>choose(p[0])} className="pathway-select">{active?"✓ Selected":selected?"Locked":"Choose pathway"}</button></article>})}</div><div className="sequence-road glass-panel"><div><span className="section-kicker">YOUR PROGRESS</span><h2>Sequence 9 → Sequence 0</h2><p>{hours} hours studied · {data.sessions||0} sessions · {days} active days</p></div><small>Sequences require progressively harder challenges and study milestones.</small></div></main>
}
function Syllabus(){
