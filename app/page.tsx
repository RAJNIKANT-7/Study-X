...
export default function Page(){
 const path=usePathname(),router=useRouter(),[loginOpen,setLoginOpen]=useState(false),[account,setAccount]=useState<string|null>(null),isAch=path==="/achievements",isProgress=path==="/progress",isPathways=path==="/pathways";
 useEffect(()=>{fetch("/api/auth").then(r=>r.json()).then(x=>setAccount(x.loggedIn?x.username:null)).catch(()=>{});const f=()=>fetch("/api/auth").then(r=>r.json()).then(x=>setAccount(x.loggedIn?x.username:null));window.addEventListener("studyx-login",f);window.addEventListener("studyx-logout",f);return()=>{window.removeEventListener("studyx-login",f);window.removeEventListener("studyx-logout",f)}},[]);
 const nav=["Timer","Progress","Pathways","Leaderboard","AI Helper"],routes:Record<string,string>={Timer:"/timer",Progress:"/progress",Pathways:"/pathways",Leaderboard:"/leaderboard","AI Helper":"/ai-helper"};
 return <div className="study-x-app"><div className="grain"/><div className="ambient-glow"/>
 <nav className="top-nav"><button className="brand" onClick={()=>router.push("/timer")}><span className="brand-mark">SX</span><b>Study<span> X</span></b></button><div className="nav-pill glass-pill">{nav.map(n=><button key={n} onClick={()=>router.push(routes[n])} className={(n==="Pathways" && isPathways) || (n==="Progress" && isProgress) || (n==="Timer" && path==="/timer") ? "active-pill" : ""}>{n}</button>)}</div><div className="nav-right"><button className="nav-action" aria-label="Notifications">•••</button><button className="login-nav" onClick={()=>setLoginOpen(true)}>{account?"@"+account:"Login"}</button></div></nav>
 <div className="content">{isAch?<Achievements/>:isProgress?<Progress/>:isPathways?<Pathways/>:<Timer/>}</div>{loginOpen&&<LoginPanel onClose={()=>setLoginOpen(false)}/>}<div className="status-bar"><span><i/> Systems normal</span><span>Study X · 2026</span></div></div>
function Progress(){
