...
function Timer(){
 ...
 return <main className="timer-page">
  ...
  <div className="bottom-tools"><button onClick={()=>document.getElementById("task-list")?.scrollIntoView({behavior:"smooth"})}>Tasks</button><button onClick={()=>setSound(v=>!v)}>{sound?"Sound on":"Sound off"}</button><button>Ambient</button><button>Study room</button></div>
