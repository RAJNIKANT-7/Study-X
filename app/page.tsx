...
function PathwayProgress(){
 ...
 async function choose(name:string){if(selected)return;const p={...data,pathway:name,sequence:9};setSelected(name);setData(p);localStorage.setItem("study-x-progress",JSON.stringify(p));try{await fetch("/api/sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({progress:p,tasks:[]})})}catch{}}
 ...
 {active&&<section className="sequence-ladder glass-panel"><div className="detail-head"><div><span className="section-kicker">{active.id.toUpperCase()} PATHWAY</span><h2>Sequence Ladder</h2></div><span>10 SEQUENCES</span></div><div className="sequence-list">{active.seq.map((name,i)=>{const n=9-i;return <div key={name} className={"sequence-item "+(n===currentSeq?"current-sequence ":"")+(n<currentSeq?"completed-sequence":"")}><span className="sequence-number">SEQUENCE {n}</span><div><b>{name}</b><small>{n===0?"TRUE GOD":n<=1?"ANGEL":n<=3?"SAINT":"BEYONDER"}</small></div><span className="sequence-status">{n===currentSeq?"CURRENT":n<currentSeq?"COMPLETED":"LOCKED"}</span></div>})}</div></section>}
      ...
      {!selected ? (
        ...
        <>
          ...
          <section className="sequence-panel glass-panel">
            ...
            <div className="sequence-ladder">{[9,8,7,6,5,4,3,2,1,0].map(seq=><div className={"seq-node "+(seq===sequence?"current":"locked")} key={seq}><span>SEQ {seq}</span><div><b>{(pathways.find(p=>p.id===selected)?.seq?.[9-seq]||(["Seer","Clown","Magician","Faceless","Marionettist","Bizarro Sorcerer","Scholar of Yore","Miracle Invoker","Attendant of Mysteries","The Fool"][9-seq]))}</b><small>{seq===sequence?"CURRENT":"LOCKED"} · {thresholds[9-seq]||0}h required</small></div><strong>{seq===sequence?"ACTIVE":"LOCKED"}</strong></div>)}</div>
