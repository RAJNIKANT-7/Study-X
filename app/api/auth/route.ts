...
export async function POST(req:NextRequest){try{const {username,password}=await req.json();const accounts=JSON.parse(process.env.STUDY_X_ACCOUNTS||"[]") as {username:string,password:string}[];const a=accounts.find(x=>x.username===String(username||"").trim()&&x.password===String(password||""));if(!a)return NextResponse.json({error:"Invalid username or password"},{status:401});const res=NextResponse.json({ok:true,username:a.username});res.cookies.set("study_x_session",token(a.username),{httpOnly:true,secure:true,sameSite:"lax",path:"/",maxAge:2592000});return res}catch{return NextResponse.json({error:"Login failed"},{status:400})}}
